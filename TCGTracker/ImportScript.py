...
def deckUpload(text):
    ...
    for card in PokemonInDeck:
        cursor.execute("""INSERT INTO \"DeckCard\" (dc_deckid, dc_qtylist, dc_qtydeck, dc_name, dc_set, dc_reg, dc_type, dc_setnumber) 
                           VALUES (%s, %s, %s, %s, %s, %s, %s, %s)""", (1, card["QTYinDeck"], 0, card["Name"], card["Set"], card["Regulation"], "Pokemon", card["Number"]))
        conn.commit()

    for card in TrainerInDeck:
        cursor.execute("""INSERT INTO \"DeckCard\" (dc_deckid, dc_qtylist, dc_qtydeck, dc_name, dc_set, dc_reg, dc_type, dc_setnumber) 
                           VALUES (%s, %s, %s, %s, %s, %s, %s, %s)""", (1, card["QTYinDeck"], 0, card["Name"], card["Set"], card["Regulation"], "Trainer", card["Number"]))
        conn.commit()

    for card in EnergyInDeck:
        cursor.execute("""INSERT INTO \"DeckCard\" (dc_deckid, dc_qtylist, dc_qtydeck, dc_name, dc_set, dc_reg, dc_type, dc_setnumber) 
                           VALUES (%s, %s, %s, %s, %s, %s, %s, %s)""", (1, card["QTYinDeck"], 0, card["Name"], card["Set"], card["Regulation"], "Energy", card["Number"]))
        conn.commit()
